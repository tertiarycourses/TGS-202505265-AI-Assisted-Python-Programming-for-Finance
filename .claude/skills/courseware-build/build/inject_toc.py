...
def main():
    ...
    # 5) Build the TOC entries INSIDE a real Word TOC field, so the document keeps a
    #    live, updatable field (F9 / update-on-open) while still carrying correct
    #    static page numbers for viewers — such as LibreOffice in the headless
    #    render — that never execute the field. Deleting the field outright would
    #    leave hard-typed page numbers that go stale on any repagination.
    anchor = placeholder._p
    def _fld(kind, instr=None):
        el = anchor.makeelement(qn('w:r'), {})
        fc = anchor.makeelement(qn('w:fldChar'), {})
        if instr is None:
            fc.set(qn('w:fldCharType'), kind); el.append(fc)
        return el
    def _fld_run(kind):
        r = anchor.makeelement(qn('w:r'), {})
        fc = anchor.makeelement(qn('w:fldChar'), {}); fc.set(qn('w:fldCharType'), kind)
        r.append(fc); return r
    def _instr_run(text):
        r = anchor.makeelement(qn('w:r'), {})
        it = anchor.makeelement(qn('w:instrText'), {})
        it.set(qn('xml:space'), 'preserve'); it.text = text
        r.append(it); return r
    # field begin + instruction + separate, in their own paragraph before the entries
    begin_p = anchor.makeelement(qn('w:p'), {})
    anchor.addprevious(begin_p)
    begin_p.append(_fld_run('begin'))
    begin_p.append(_instr_run(' TOC \\o "1-3" \\h \\z \\u '))
    begin_p.append(_fld_run('separate'))
    ...
    # field end, closing the TOC field around the static entries above
    end_p = anchor.makeelement(qn('w:p'), {})
    anchor.addprevious(end_p)
    end_p.append(_fld_run('end'))
