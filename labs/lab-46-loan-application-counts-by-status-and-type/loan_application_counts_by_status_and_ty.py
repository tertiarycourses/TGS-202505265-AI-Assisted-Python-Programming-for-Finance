...
You will build: A loan_counts.py script producing a Loan_Type x Loan_Status count table plus an approval-rate table.
Tools: uv, pandas, VS Code / Cursor AI assistant

Run this lab with uv:
    uv run python loan_application_counts_by_status_and_ty.py
"""


def main():
    print("Lab 46: Loan Application Counts by Status and Type")
    ...
    # ---- Step 6 -----------------------------------------------------------
    # Prompt your AI assistant: "Using pandas, build a crosstab of Loan_Type
    # against Loan_Status from df_finance, then produce a second table
    # normalised by row so each cell is the percentage of that loan type's
    # applications in that status, rounded to 1 decimal. Print which Loan_Type
    # has the lowest Approved percentage and which has the highest Pending
    # percentage."
    # TODO: implement this step

    # ---- Step 7 -----------------------------------------------------------
    # Save it as loan_counts.py, review the normalize='index' argument, and
    # confirm each row sums to 100%.
    # TODO: implement this step
    ...
    # ---- Step 10 ----------------------------------------------------------
    # Discuss: why is an approval rate a more actionable KPI for a credit
    # committee than a raw approval count?
    # TODO: implement this step

    # ---- Test it ----------------------------------------------------
    # Every row of the normalised table sums to 100.0 (+/- 0.1) and the crosstab
    # margin total equals the DataFrame row count.
